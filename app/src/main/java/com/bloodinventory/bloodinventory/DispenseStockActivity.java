...
public class DispenseStockActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_dispense_stock);
        ...
        saveButton.setOnClickListener(v -> saveDispense());
    }
    ...
    private void saveDispense() {
        ...
        dispense.put("date", date);
        dispense.put("itemName", itemName);
        dispense.put("quantity", Integer.parseInt(quantity));
        dispense.put("unit", unit);
        dispense.put("lotNumber", lotNumber);
        dispense.put("expireDate", expireDate);
        dispense.put("notes", notes);
        dispense.put("userId", user.getUid());
        dispense.put("createdAt", System.currentTimeMillis());
        dispense.put("type", "dispense");

        db.collection("stock")
                .add(dispense)
                .addOnSuccessListener(documentReference -> {
                    Toast.makeText(DispenseStockActivity.this, "تم صرف المخزون بنجاح", Toast.LENGTH_SHORT).show();
                    clearFields();
                })
                .addOnFailureListener(e -> {
                    Toast.makeText(DispenseStockActivity.this, "فشل صرف المخزون: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                });
    }
