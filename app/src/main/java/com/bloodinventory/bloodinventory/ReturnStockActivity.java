...
public class ReturnStockActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        saveButton.setOnClickListener(v -> saveReturn());
    }
    ...
    private void saveReturn() {
        ...
        Map<String, Object> returnStock = new HashMap<>();
        returnStock.put("date", date);
        returnStock.put("itemName", itemName);
        returnStock.put("quantity", Integer.parseInt(quantity));
        returnStock.put("unit", unit);
        returnStock.put("lotNumber", lotNumber);
        returnStock.put("expireDate", expireDate);
        returnStock.put("userId", user.getUid());
        returnStock.put("createdAt", System.currentTimeMillis());
        returnStock.put("type", "return");

        db.collection("stock")
                .add(returnStock)
                .addOnSuccessListener(documentReference -> {
                    Toast.makeText(ReturnStockActivity.this, "تم إضافة المرتجع بنجاح", Toast.LENGTH_SHORT).show();
                    clearFields();
                    // Schedule notification check
                    ExpirationNotificationService.scheduleNotificationCheck(this);
                })
                .addOnFailureListener(e -> {
                    Toast.makeText(ReturnStockActivity.this, "فشل إضافة المرتجع: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                });
    }
